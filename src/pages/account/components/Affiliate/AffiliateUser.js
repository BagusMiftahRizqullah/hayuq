...
const AffiliateUser = observer((props) => {
  ...
  useEffect(() => {
    ...
    if (from == 'Leader') {
      ...
      const newData = dataDetail?.detail?.filter((a) => a.user_type == from);
      console.log('newDataSSS', newData);
      setRefferalCommision(newData);
    }

    // if (from == 'user' && dataDetail?.referral?.type == '1') {
    //   const newData = dataDetail?.detail?.filter((a) => a.user_type == 'Users');
    //   setRefferalCommision(newData);
        ...
        <HStack style={componentStyles.navigatorBarContainer}>
          <HStack>
            ...
            <VStack style={{alignSelf: 'center'}}>
              <Text style={[Fonts.subhead]}>User Referral</Text>
            </VStack>
          </HStack>
