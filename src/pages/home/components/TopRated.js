...
const TopRated = observer(() => {
  ...
  useEffect(() => {
    if (HomeStore?.listExploreHayuqersData !== null) {
      setTopRate(HomeStore?.listExploreHayuqersData.slice(0, 10));
    }
  }, [HomeStore?.listExploreHayuqersData]);

  const goToExploreMenuDetail = async (id) => {
    if (await CekUserToken()) {
      HomeStore.setRouterFilter('Top rated', 'normal');
  ...
  const goToRestaurantPage = async (id) => {
    if (await CekUserToken()) {
      HomeStore.setMerchantDetailId(id);
    ...
    <VStack>
      <HStack style={componentStyles.titleRecomended}>
        ...
        <Button
          ...
          disabled={HomeStore.listExploreLoading}
        />
      </HStack>
      <FlatList
        style={{
